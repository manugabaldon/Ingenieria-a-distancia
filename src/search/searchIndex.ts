...
import type { CourseModule } from '../courses/CourseView';
...
export type SearchTarget =
  ...
  | { kind: 'view'; id: string }
  | { kind: 'chapter'; modId: string; chId: string };
...
/** Construye el índice de teoría, ejercicios resueltos y capítulos del temario. */
export function buildContentSearchIndex(opts: {
  ...
  for (const mod of opts.courseModules) {
    const modLabel = `${mod.code} · ${mod.title}`;
    for (const ch of mod.chapters) {
      items.push({
        id: `chapter-${mod.id}-${ch.id}`,
        title: ch.title,
        subtitle: modLabel,
        icon: '🎓',
        badge: 'Temario',
        keywords: normalize(`${ch.title} ${mod.title} ${mod.code} ${mod.subtitle}`),
        target: { kind: 'chapter', modId: mod.id, chId: ch.id },
      });
    }
  }
