...
// ─── Physics ──────────────────────────────────────────────────────────────────
// Campo de velocidades:  ẋ = −(v/a)·y,  ẏ = (v/a)·x   →  ω = v/a
// Ecuaciones horarias:   x = a·cos(ωt),  y = a·sin(ωt)
// Velocidad (tangente, módulo v):  vx = −v·sin(ωt),  vy = v·cos(ωt)
...
export default function CircularCP5() {
  ...
  function draw(t: number) {
    ...
    // ── Velocity field arrows (rotación pura) ──────────────────────────────────
    if (showField) {
      ctx.save();
      const step = a;                 // muestreo cada "a" metros
      const maxR = Math.ceil(7 / step) * step;
      for (let gx = -maxR; gx <= maxR; gx += step) {
        for (let gy = -maxR; gy <= maxR; gy += step) {
          if (gx === 0 && gy === 0) continue;
          const fx = -w * gy, fy = w * gx;          // V = (−ω y, ω x)
          const mag = Math.hypot(fx, fy);
          const L = 14;                              // longitud fija del glifo
          const px = OX + gx * SC, py = OY - gy * SC;
          const ux = fx / mag, uy = -fy / mag;       // dirección (canvas y invertida)
          arrow(ctx, px, py, px + ux * L, py + uy * L, 'rgba(99,102,241,0.35)', '', 1.4);
        }
      }
      ctx.restore();
    }
    ...
    <div className="sim-body">
      <div className="canvas-area">
        ...
        <div className="legend">
          ...
          <label className="legend-item" style={{ cursor: 'pointer' }}>
            ...
            <span className="legend-dot" style={{ background: '#6366f1' }} />Campo V
          </label>
        </div>
      </div>
      ...
      <div className="params-panel">
        ...
        <div className="info-box" style={{ marginTop: '0.75rem', fontSize: '0.8rem' }}>
          ...
          • La velocidad es siempre <strong>tangente</strong> y de módulo constante v<br />
          • La aceleración apunta siempre al <strong>centro</strong> (centrípeta)<br />
          • El campo V es perpendicular a r → rotación pura<br />
          • Activa <strong>Campo V</strong> para ver el campo de velocidades completo
        </div>
      </div>
    </div>
  );
